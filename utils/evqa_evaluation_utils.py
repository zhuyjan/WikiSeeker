...
_VOCAB_PATH = './tf_cache/vocab.txt'
_MODEL_PATH = './tf_cache/answer_equivalence_bem_1'
...
# 4 - Define and return scoring function
  def score_example_with_verbosity(
      example,
      threshold_score = True,
  ):
    """Scores an Encyclopedic-VQA example using BEM.
    ...
    return score, 0, logits

  return score_example, score_example_with_verbosity
...
def initialize_encyclopedic_vqa_evaluation_function(
  ...
  bem_scoring_function, _ = initialize_bem_scoring_function(
...
def evaluate_example(
  ...
  Args:
    ...
    with_verbose: Whether to print verbose information.
  Returns:
  ...
  bem_scoring_function, bem_scoring_function_with_verbosity = initialize_bem_scoring_function()
  ...
  if not with_verbose:
    ...
    for reference in reference_list:
      ...
      # Replicate encyclopedic logic for verbose mode: EM first, else BEM with raw logits
      score = bem_scoring_function_with_verbosity(example, threshold_score=False)
      scores.append(score)
    return max(scores)
